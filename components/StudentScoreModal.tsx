...
// ── Main Component ──────────────────────────────────────────────────────────
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: profileData, error: profileError } = await supabase
        .from('profiles')
        .select('id, email, student_id, student_name, class_number')
        .eq('role', 'student');
  ...
  // Filter students by search keyword (student_id, student_name, email, class_number)
  const filteredStudents = useMemo(() => {
    ...
    return studentScores.filter(({ student }) => {
      ...
      const classNumber = student.class_number?.toLowerCase() ?? '';
      ...
      >
        ...
        <div className="px-6 pb-4 flex-shrink-0">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="搜索学号、姓名、班级号或邮箱..."
              value={searchKeyword}
              onChange={(e) => setSearchKeyword(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/40 focus:outline-none focus:border-purple-500/50 focus:bg-white/10 transition-all text-sm"
            />
            {searchKeyword && (
              <button
                onClick={() => setSearchKeyword('')}
                className="absolute inset-y-0 right-0 pr-3 flex items-center hover:text-white transition-colors text-white/40"
              >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 pb-6">
          {loading ? (
              ...
              <div className="text-center">
                {searchKeyword.trim() ? (
                  <>
                    ...
                    <p className="text-white/40 font-medium">未找到匹配的学生</p>
                    <p className="text-white/30 text-sm mt-1">尝试使用其他关键词搜索</p>
                  </>
                ) : (
                  <>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mx-auto mb-4 text-white/20" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="space-y-2 mt-2">
              {/* Table Header */}
              <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-4 px-4 py-2 text-xs font-semibold text-white/50 uppercase tracking-wider">
                      ...
                      {sortedStudents.map(({ student, totalScore, lessonDetails, completedLessons, totalLessons }) => {
                        ...
                        return (
                          <div key={student.id} className="rounded-xl border border-white/10 overflow-hidden transition-all">
                            {/* Student Summary Row */}
                            <div
                              className={`grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-4 px-4 py-3 items-center cursor-pointer transition-colors ${isExpanded ? 'bg-purple-500/10 border-b border-white/10' : 'bg-white/5 hover:bg-white/8'}`}
